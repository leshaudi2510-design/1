...
// A wrapped sweet and a round multiplier bomb, its fuse ending in a spark at [x, y].
const SWEET_BOMB = (fuse, [x, y]) => `${star(28, 26, 9, '#FFF5E1')}<circle cx="104" cy="18" r="2.6" fill="#FFF5E1"/><circle cx="52" cy="114" r="2.2" fill="#FFF5E1"/>
      ...
      <path class="ink-t" d="${fuse}" fill="none"/>
      ${star(x, y, 9, '#FFE11A', 'ink-t')}
...
export const COVERS = {
  ...
  'sweet-bonanza': {
    ...
    art: SWEET_BOMB('M150 44q4-14 14-20', [166, 20]),
    // the wrapper's tip and the bomb's cap clear BONANZA, and the fuse's spark its last A
    stage: { dy: 8, art: SWEET_BOMB('M150 44q10-12 24-14', [176, 26]) },
