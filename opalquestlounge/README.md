...
└── tools/
    ...
    ├── subset-fonts.py       ← subset и обрезка осей шрифтов (из полных файлов google/fonts)
...
| `tools/check.mjs` | 360 / 768 / 1440 px без горизонтальной прокрутки; все игры пройдены только с клавиатуры; фокус не теряется; до согласия нет ни одного запроса к третьим сторонам и ни одной cookie; Consent Mode v2 — все 7 сигналов `denied`; «Reject all» и «Accept all» визуально равны; gtag.js грузится только после «Accept all»; баннер cookies первым в Tab-порядке и не закрывает фокус; раздел `chrome`: перерыв и дневной лимит (в том числе в полночь) сами снимаются в открытой вкладке, лимит в Settings сохраняется только кнопкой, фокус не прячется под доком, шапка без прокрутки вбок при большом балансе и с интервалами WCAG 1.4.12, вопрос о возрасте до загрузки app.js, страница без JavaScript, CLS < 0.05 при задержанных шрифтах |
